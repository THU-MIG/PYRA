...
from model.utils import even
from model.adaptive_merge import init_pyra_scale
...
class Adapter(nn.Module):
    def __init__(self,
        ...
        self.reduction_dim = reduction_dims
        self.dropout = nn.Dropout(p=drop_rate_adapter)

        print('adapter',self.reduction_dim)
        if self.reduction_dim > 0:
            self.ln1 = nn.Linear(self.embed_dims, self.reduction_dim)
            self.activate = QuickGELU()
            self.ln2 = nn.Linear(self.reduction_dim, self.embed_dims)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm,visual_prompt_dim=1024,LoRA_dim=1024,adapter_dim=1024,prefix_dim=1024,drop_rate_LoRA=0,drop_rate_prompt=0,drop_rate_adapter=0,last_prompt_dim=0,
                 pyra_r=None):
        super().__init__()
        ...
        # prompt setting
        self.visual_prompt_dim = visual_prompt_dim
        ...
        self.last_prompt_dim = last_prompt_dim
        ...
        self.pyra_r = pyra_r
        if self.pyra_r is not None and self.pyra_r > 0:
            self.pyra_a = init_pyra_scale(self.pyra_r, 1, init="sqrt")
            self.pyra_b = init_pyra_scale(1, dim, init="zero")
            self.pyra_norm = nn.LayerNorm(dim)
            self.pyra = (self.pyra_a, self.pyra_b, self.pyra_norm)
        else:
            self.pyra = None
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='',prompt_tuning_dim=1024,LoRA_dim=1024,adapter_dim=1024,prefix_dim=1024,
                 drop_rate_LoRA=0,drop_rate_prompt=0,drop_rate_adapter=0, IS_not_position_VPT=False,
                 pyra_r=None):
        """
        Args:
        ...
        self.pyra_r = pyra_r
        if self.pyra_r is not None:
            assert len(pyra_r) == depth
        else:
            self.pyra_r = [None] * depth
        ...
        self.blocks = nn.Sequential(*[
            Block(
                ...
                visual_prompt_dim=prompt_tuning_dim,LoRA_dim=LoRA_dim,adapter_dim=adapter_dim,prefix_dim=prefix_dim,
                drop_rate_LoRA=drop_rate_LoRA,drop_rate_prompt=drop_rate_prompt,drop_rate_adapter=drop_rate_adapter,last_prompt_dim=0 if i == 0 else prompt_tuning_dim,
                pyra_r=self.pyra_r[i]
            )
        ...
        self.prompt_tuning_dim = prompt_tuning_dim
        if prompt_tuning_dim > 0:
            self.visual_prompt_token = nn.Parameter(torch.zeros(1, prompt_tuning_dim, embed_dim))
            nn.init.xavier_uniform_(self.visual_prompt_token)
            if not IS_not_position_VPT:
                self.visual_prompt_token_pos_embed = nn.Parameter(torch.zeros(1, prompt_tuning_dim, embed_dim))
    ...
    def freeze_stages(self):
        ...
        for name,param in self.named_parameters():
            if 'adapter' not in name and 'prompt' not in name and 'LoRA' not in name and 'prefix' not in name and 'head' not in name and 'pyra' not in name:
        ...
        for name,param in self.named_parameters():
            ...
            if param.requires_grad:
                ...
                if 'adapter' in name:
                    ...
                    pyra_para_nums += param.numel()
                
                
        print('total training parameters:',training_para_nums,'adapter',adapter_para_nums,'LoRA',LoRA_para_nums,'prompt',vp_para_nums,'prefix',prefix_para_nums,'PYRA',pyra_para_nums,'head',head_para_nums)
        print('total parameters in model:',model_para_nums)
    ...
    def forward_features(self, x):
        ...
        if self.prompt_tuning_dim > 0:
            visual_prompt_tokens = self.visual_prompt_token.expand(B,-1,-1)
            if not self.IS_not_position_VPT:
                visual_prompt_tokens = visual_prompt_tokens + self.visual_prompt_token_pos_embed
            visual_prompt_tokens = self.drop_prompt(visual_prompt_tokens)
            x = torch.cat((x,visual_prompt_tokens), dim=1)
    ...
    def forward_features_for_output(self, x):
...
def _load_weights(model: VisionTransformer, checkpoint_path: str, prefix: str = ''):
    ...
    for i, block in enumerate(model.blocks.children()):
        ...
        if not hasattr(block.attn.qkv, 'fc_origin'):
            ...
            block.attn.qkv.fc_origin.weight.copy_(torch.cat([
                _n2p(w[f'{mha_prefix}{n}/kernel'], t=False).flatten(1).T for n in ('query', 'key', 'value')]).unsqueeze(-1))
            block.attn.qkv.fc_origin.bias.copy_(torch.cat([
                _n2p(w[f'{mha_prefix}{n}/bias'], t=False).reshape(-1) for n in ('query', 'key', 'value')]))
            block.attn.proj.fc_origin.weight.copy_(_n2p(w[f'{mha_prefix}out/kernel']).flatten(1).unsqueeze(-1))
            block.attn.proj.fc_origin.bias.copy_(_n2p(w[f'{mha_prefix}out/bias']))
            for r in range(2):
                getattr(block.mlp, f'fc{r + 1}').fc_origin.weight.copy_(_n2p(w[f'{block_prefix}MlpBlock_3/Dense_{r}/kernel']).unsqueeze(-1))
                getattr(block.mlp, f'fc{r + 1}').fc_origin.bias.copy_(_n2p(w[f'{block_prefix}MlpBlock_3/Dense_{r}/bias']))
